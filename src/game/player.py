...
class Player:
    ...
    def update(self, dt: float, tiles: list[Rect], keys: pygame.key.ScancodeWrapper) -> None:
        ...
        # Hardcoding some frame ranges for animation
        # Assuming run is at row 1, idle at row 0, jump at row 2
        # (This can be modified when exact spritesheet layout is known)
        frames_per_state = {"idle": 4, "run": 6, "jump": 2}
        
        self.anim_timer += dt
        if self.anim_timer > 0.15:
            self.anim_timer = 0
            self.current_frame = (self.current_frame + 1) % frames_per_state[self.state]

    def render(self, renderer: Renderer) -> None:
        # Determine UV coordinates based on state
        row = 0
        if self.state == "run":
            row = 1
        ...
        if not self.facing_right:
            # Flip horizontally using UV coordinates
            uv_x += uv_w
            uv_w = -uv_w
